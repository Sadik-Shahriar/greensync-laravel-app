...
const DashboardHome: React.FC = () => {
  ...
  const currentProgress = user?.eco_points ? Math.min((user.eco_points / nextLevelPoints) * 100, 100) : 0;
              ...
              <CardContent>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground">Level {Math.floor((user?.eco_points || 0) / 200) + 1}</span>
                    <span className="text-xs font-bold">{user?.eco_points || 0} / {nextLevelPoints} points</span>
                  </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-bold">{stats?.disposals ?? 0}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-muted-foreground">Points Earned</span>
                  <span className="text-xs font-bold">{stats?.eco_points ?? 0}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-muted-foreground">Waste Saved</span>
                  <span className="text-xs font-bold">{stats?.waste_saved ?? 0} kg</span>
                </div>
              </CardContent>
            </Card>
